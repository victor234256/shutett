...
const Linkcard = ({ url = [], fetchURL }) => {
	...
	const handleCopy = () => {
		navigator.clipboard
			.writeText(
				`https://shutett.vercel.app/${url?.short_url}`,
			)
			.then(() => {
				setIsCopied(true);
	...
	const { loading: deleteLoader, fn: deleteFunction } =
		useFetch(deleteUrl, url.id);

	return (
		<div className="relative dark:text-white flex flex-col md:flex-row gap-5 dark:bg-gray-900 rounded-lg border p-4 bg-white text-gray-900 shadow-xl">
			<img
				src={url?.qrcode}
			...
			<span className="flex flex-col gap-2 flex-1">
				<span className="text-2xl hover:underline font-bold">
					{url?.title}
				</span>
